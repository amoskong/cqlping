...
./cqlping scylla-server \
  ...
  --verify-query "select * from keyspace1.standard1 where key=textAsBlob('1')" -c 2 -s 10 --debug
...
./cqlping scylla-server \
  ...
  --verify-query "select * from keyspace1.standard1 where key=textAsBlob('1')" -c 2 --debug
...
./cqlping scylla-server \
  ...
  --verify-query "select * from cqlping.cf_txt where key1=1" -c 2 -s 2 --debug
...
./cqlping scylla-server \
  ...
  --verify-query "select * from cqlping.cf_blob where key1=1" -c 2 -s 2 --debug
